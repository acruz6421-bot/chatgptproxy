...
* ✅ **Funciona bem — agentes de CLI leves:** validado com o **Hermes** executando tarefas agênticas reais (listar e ler arquivos e então responder), com *tool-calling* de ponta a ponta e sem alucinação de ambiente; e com o **openclaw** (turno de agente via provider customizado, resposta correta de ponta a ponta). Clientes nesse estilo — prompt enxuto e tolerantes a pequenas variações na *tool call* — são a melhor escolha. Basta registrar o proxy como provider OpenAI-compatible (`base_url` da porta do proxy, key `chatgpt-local-dev`).
